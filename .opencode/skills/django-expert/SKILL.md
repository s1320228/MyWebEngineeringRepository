---
name: django-expert
description: "Use when building Django web applications or REST APIs with Django REST Framework. Invoke when working with settings.py, models.py, manage.py, views, serializers, or any Django project file. Creates Django models with proper indexes, optimizes ORM queries using select_related/prefetch_related, builds DRF serializers and viewsets, and configures authentication."
license: MIT
...
2. **Design models** — Create models with proper fields, indexes, managers → run `python manage.py makemigrations` and `python manage.py migrate`; verify schema before proceeding
...
6. **Test** — Django TestCase, APITestCase, pytest-django
...
- Use environment variables for secrets (python-dotenv or django-environ)
